...
class NeobotixSchunkGymEnv(gym.Env):
    ...
    def reset_again(self):
        ...
        self.dis_init = np.linalg.norm(self._observation[0:3])  #the distance to goal at beginning
    ...
    def _reward(self):
        ...
        #
        #     return reward
        delta_dis = self.ee_dis - self._dis_vor #how much is the distance to goal reduced
        self._dis_vor = self.ee_dis  #distance between End effector and goal at last step

        tau = (self.ee_dis/self.dis_init)**2
        if tau > 1:  #distance to goal becomes bigger than init
    ...
    def _sample_action(self):
        if not self._isDiscrete:  #if the action space is continue
